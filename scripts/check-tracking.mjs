...
const recharge={href:'https://fe.dtyuedan.cn/shop/panghu?utm_content=home__hero_recharge',dataset:{cta:'hero_recharge',product:'recharge'}};
handlers.click({type:'click',target:{closest:()=>recharge}});
assert.equal(calls.length,3,'self-service recharge counts once');
assert.equal(calls[2][2].link_url,'https://fe.dtyuedan.cn/shop/panghu');
assert.equal(calls[2][2].cta_position,'hero_recharge');
handlers.click({type:'click',target:{closest:()=>({...recharge,href:'https://fe.dtyuedan.cn/shop/other'})}});
assert.equal(calls.length,3,'other shops must not count');
